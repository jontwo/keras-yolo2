...
class YOLO(object):
    def __init__(self, backend,
        ...
        if backend == 'Inception3':
            ...
            raise Exception(
                'Architecture not supported! Only support Full Yolo, Tiny Yolo, MobileNet, '
                'SqueezeNet, VGG16, ResNet50, and Inception3 at the moment!')
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        # penalize the confidence of the boxes, which are reponsible for corresponding
        # ground truth box
        ...
        loss_conf = tf.reduce_sum(tf.square(true_box_conf - pred_box_conf) * conf_mask) / (
                nb_conf_box + SMOOTH_FACTOR) / 2.
        loss_class = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=true_box_class,
                                                                    logits=pred_box_class)
        loss_class = tf.reduce_sum(loss_class * class_mask) / (nb_class_box + SMOOTH_FACTOR)
        ...
        if self.debug:
            ...
            current_recall = nb_pred_box / (nb_true_box + SMOOTH_FACTOR)
        ...
        for i in range(generator.size()):
            ...
            if len(pred_boxes) > 0:
                pred_boxes = np.array([[box.xmin * raw_width, box.ymin * raw_height,
                                        box.xmax * raw_width, box.ymax * raw_height, box.score] for
                                       box in pred_boxes])
            else:
                pred_boxes = np.array([[]])
